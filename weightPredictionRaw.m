...
data = xlsread('rawData.xlsx'); % read the excel file "rawData.xlsx"
...
for i=1:12 % for each month
	...
	inputs = [inputs; data(:, ((i-1)*4)+1 : ((i-1)*4)+3)]; % extract inputs
	targets = [targets; data(:, 4*i)]; % extract target
end

delete = [];
for i=1:size(targets,1)

	if targets(i) * false ~= 0
		delete = [delete, i]; % mark NaN's for deletion
	end		
end

index = 0;
for i=delete
	targets(i - index) = []; % delete NaN's
	inputs(i - index, :) = [];
	index = index + 1;
end
